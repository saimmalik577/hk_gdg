...
import logging
import numpy as np
import sys # To check if paths exist before running
...
MODEL_PATH = Path("./models/layoutlmv3_finetuned_smartdoc_qa/checkpoint-1875")
...
class TokenizedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Return the dictionary of tensors directly
...
def evaluate_model(model_path: Path, test_data_path: Path, batch_size: int):
    ...
    (Paths and batch_size are passed in, but determined by hardcoded values above)

    Args:
        model_path: Path to the fine-tuned model checkpoint directory.
        test_data_path: Path to the preprocessed and tokenized test data (.pt file).
        batch_size: Batch size for evaluation.
    """
    logging.info(f"--- Starting Evaluation ---")
    logging.info(f"Using Hardcoded Model Path: {model_path}")
    logging.info(f"Using Hardcoded Test Data Path: {test_data_path}")
    logging.info(f"Using Hardcoded Batch Size: {batch_size}")

    # --- 1. Setup Device ---
    # Automatically use MPS if available on Mac, otherwise CPU
    if torch.backends.mps.is_available():
        device = torch.device("mps")
    elif torch.cuda.is_available(): # Keep CUDA check just in case
    ...
    # --- 2. Load Model ---
    # No need to load Processor if data is already tokenized
    try:
    ...
    try:
        ...
        tokenized_test_samples = torch.load(test_data_path, map_location='cpu') # Load to CPU first
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader, desc="Evaluating"):
            # Prepare batch inputs and move to the selected device
            try:
    ...
    logging.info("\n--- Evaluation Results ---")
    try:
        # Ensure list of lists format is correct
...
if __name__ == "__main__":
    ...
    if paths_ok:
        # Call the evaluation function with the hardcoded values
        evaluate_model(
